    ...
    class CreatePlayerPatch
    {
        ...
        public static void OnComebackMessage(byte Sendto = byte.MaxValue)
        {
            if (Main.UseingJapanese)
            {
                ...
                Utils.SendMessage($"{text}{text2}{text3}{text4}", Sendto, $"<{Main.ModColor}>【This Room Use \"Town Of Host-Temp\"】</color>");
            }
            else
            {
                var varsion = Main.PluginShowVersion;
                ...
                Utils.SendMessage($"{text}{text2}", Sendto, $"<{Main.ModColor}>【This Room Use \"Town Of Host-Temp\"】</color>");
            }
